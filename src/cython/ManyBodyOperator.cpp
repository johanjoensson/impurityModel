...
bool ManyBodyOperator::state_is_within_restrictions(
  ...
  for (const auto &[groups, bounds] : m_weighted_restrictions_mask) {
    ...
    namespace {
    // boost::unordered_flat_map is an open-addressing (flat) hash map:
    // contiguous storage, no per-node allocation, far better cache behavior
    // than std::unordered_map for the apply accumulator. boost::hash does not
    // pick up the std::hash<SlaterDeterminant> specialization in
    // SlaterDeterminant.h, so wrap it explicitly.
...
#if defined(PARALLEL)
    // Maximum threads the apply path may use. Prefer OMP_NUM_THREADS (the
    // cores the user requested *this* MPI task) so one rank per task never
    // oversubscribes the node; fall back to single thread only when
    // unset. Resolved once.
    ...
    [[nodiscard]] ManyBodyState ManyBodyOperator::apply(
        const ManyBodyState &state, double cutoff) const {
      std::vector<std::pair<ManyBodyState::Key, ManyBodyState::Value>>
          local_res;
      ...
      // Phase 5b: scale the thread count to the workload instead of always
      // grabbing every core. Small states (few SDs) don't amortize thread spawn
      // + the bucket merge, and under MPI every rank would otherwise
      // oversubscribe the node; require >= MIN_SD_PER_THREAD input determinants
      // per thread, so tiny applies run on a single thread.
      ...
      // Output is partitioned across `num_buckets` by key hash so the merge
      // below runs one lock-free thread per bucket (disjoint key sets), instead
      // of the old serial merge that re-did every insert on one thread and
      // capped the speedup.
      const unsigned int num_buckets = num_threads;
      const SlaterKeyHash hasher;
      // local_buckets[t * num_buckets + b]: outputs from compute-thread t
      // hashing to bucket b.
      ...
      for (unsigned int t = 0; t < num_threads; t++) {
        ...
        threads.push_back(std::thread([&, t, start_slater, end_slater]() {
          ...
          for (; it != end; it++) {
            ...
            // Copy the input determinant ONCE per SD; each term applies in
            // place and undoes its operators afterwards (Phase 1a), instead of
            // copying per term.
            ...
            for (size_t op_idx = 0; op_idx < m_flat_coeffs.size(); op_idx++) {
              // Phase 2b fast path: pure number-operator product -> one
              // occupancy AND-test and a constant-signed scalar add, no
              // create/annihilate.
              ...
              if (m_flat_onebody[op_idx]) {
                ...
                if (bit_set(slater, ob_j) && !bit_set(slater, ob_i)) {
                  const double sgn =
                      mask_parity(slater, m_onebody_between[op_idx]) ? -1.0
                                                                     : 1.0;
              ...
              // Restore out_slater_determinant to `slater`: undo the
              // [start_idx, i) operators that actually toggled a bit.
      ...
      // Phase 5a: parallel merge -- one thread per output bucket. Bucket b owns
      // a disjoint set of keys (all hashing to b), so the threads never touch
      // the same entry and need no locks. Each thread also applies the cutoff
      // while collecting its bucket's survivors.
      std::vector<
          std::vector<std::pair<ManyBodyState::Key, ManyBodyState::Value>>>
      ...
      for (const auto &[slater, amp] : state) {
        // Copy the input determinant ONCE per SD; each term applies its
        // operators in place and undoes them afterwards (Phase 1a), instead of
        // copying per term.
        out_slater_determinant = slater;
        // Phase 2b: sum every diagonal term's contribution (all map to
        // `slater`) into one scalar and emit a single insert, instead of one
        // colliding insert per term.
        ManyBodyState::mapped_type diag_accum{0.0, 0.0};
        for (size_t op_idx = 0; op_idx < m_flat_coeffs.size(); op_idx++) {
          ...
          // operators that actually toggled a bit. A failing op (s == 0) at
          // index i did not modify the determinant, so it is excluded.
          for (size_t j = start_idx; j < i; j++) {
            const int64_t idx = m_flat_indices[j];
            toggle_bit(out_slater_determinant,
                       idx >= 0 ? static_cast<size_t>(idx)
    ...
    // (IMPURITYMODEL_PARALLEL=1 at install time). Exposed to Python so tests
    // can choose exact vs tolerance assertions: the threaded merge changes the
    ...
    [[nodiscard]] ManyBodyBlockState ManyBodyOperator::apply(
        const ManyBodyBlockState &block, double cutoff) const {
      // Block variant of the serial apply above: one pass over the shared
      // support, per-(determinant, term) work done once, p amplitudes emitted
      // per hit. The accumulator maps out-determinant -> row index of a growing
      // row-major buffer (one hash op per (det, term) instead of p). Per-column
      // arithmetic mirrors the scalar path exactly: same term order, same +=
      // sequence per column.
      if (m_flat_dirty) {
        build_flat_representation();
      }
      ...
      std::vector<ManyBodyBlockState::Value>
          acc; // row-major, row_of.size() * p
      acc.reserve(block.rows() * p);

      // Row of `k` in acc, appending a zero row on first sight. resize() grows
      // the buffer geometrically, so the amortized cost stays O(1) per new row.
...
#if defined(PARALLEL)
      // Threaded block apply (Phase 2.5): partition the input ROWS across
      // threads, one block accumulator per (thread, bucket) pair, lock-free
      // per-bucket merge — the same structure as the scalar threaded apply
      // above. Each row carries ~p times the scalar work, so the per-thread
      // workload floor shrinks accordingly.
      constexpr size_t MIN_SD_PER_THREAD = 256;
      ...
      if (num_threads > 1) {
        ...
        const size_t chunk_size =
            (block.rows() + num_threads - 1) / num_threads;
        for (unsigned int t = 0; t < num_threads; t++) {
          ...
          threads.push_back(std::thread([&, t, start_row, end_row]() {
            ...
            // Per-row term loop, duplicated from the serial body below (the
            // same precedent as the scalar threaded apply: the loop reads
            // private flat-term members, so a shared free function would need
            // an unwieldy signature).
        ...
        std::vector<std::vector<ManyBodyBlockState::Key>> bucket_keys(
            num_buckets);
        ...
        for (unsigned int b = 0; b < num_buckets; b++) {
          merge_threads.push_back(std::thread([&, b]() {
            ...
            for (auto &[k, row] : macc.row_of) {
              const ManyBodyBlockState::Value *src_row =
                  macc.acc.data() + row * p;
              for (std::size_t c = 0; c < p; ++c) {
        ...
        std::vector<std::pair<ManyBodyBlockState::Key,
                              const ManyBodyBlockState::Value *>>
            survivors;
          ...
          if (m_flat_onebody[op_idx]) {
            ...
            if (bit_set(slater, ob_j) && !bit_set(slater, ob_i)) {
              ...
              if (!check_restrictions ||
                ...
                const auto coeff = m_flat_coeffs[op_idx] * sgn;
                ManyBodyBlockState::Value *dst =
                    emit_row(out_slater_determinant);
                for (std::size_t c = 0; c < p; ++c) {
                  dst[c] += coeff * amp[c];
                }
              }
      ...
      // Keep rows where ANY column survives the cutoff, then sort by
      // determinant to establish the container's sorted-unique invariant.
    ...
    ManyBodyOperator &ManyBodyOperator::operator+=(
        const ManyBodyOperator &other) noexcept {
      m_flat_dirty = true;
      if (m_ops.empty()) {
        m_ops = other.m_ops;
        return *this;
    ...
    // Rewrite one operator string (in product / left-to-right reading order)
    // into a list of normal-ordered terms (creations before annihilations, each
    // group ascending in orbital) using the fermionic anticommutators:
    //   c_p c^d_q = delta_pq - c^d_q c_p   (contraction when p == q)
    //   c^d_p c^d_q = -c^d_q c^d_p,  c_p c_q = -c_q c_p   (p != q;
    //   equal-orbital pair = 0).
    // The recursion fixes the leftmost out-of-order adjacent pair and
    // terminates because each swap reduces the inversion count and each
    // contraction shortens the term.
    void normal_order_recurse(std::vector<int64_t> ops,
      ...
      for (size_t k = 0; k + 1 < ops.size(); k++) {
        ...
        // Pauli: two adjacent same-type operators on the same orbital
        // annihilate the term.
        ...
        const int rank_a =
            a_create ? 0 : 1; // creations sort before annihilations
    ...
    // Build the list of terms apply() flattens: the raw operator terms, or
    // their normal- ordered expansion (merged, with zero/Pauli terms dropped)
    // when normal ordering is on.
    std::vector<Term>
    collect_flat_terms(const std::vector<ManyBodyOperator::value_type> &m_ops,
      ...
      for (const auto &op : m_ops) {
        // m_ops stores operators rightmost-first (apply order); reverse to
        // product order.
      ...
      terms.erase(std::remove_if(terms.begin(), terms.end(),
                                 [](const Term &t) {
                                   return std::norm(t.second) < 1e-24;
                                 }),
                  terms.end());
      return terms;
    }
    ...
    void ManyBodyOperator::build_flat_representation() const {
      ...
      for (const auto &op : terms) {
        ...
        m_flat_indices.insert(m_flat_indices.end(), indices.begin(),
                              indices.end());
        ...
        // multisets <=> every orbital's occupation is conserved <=> diagonal.
        // The empty (constant) term has both multisets empty, so it is
        // diagonal.
        creators.clear();
        annihilators.clear();
        ...
        // Pure-number-product fast path (Phase 2b). Require balanced (==
        // diagonal) and all-distinct orbitals, then *probe*: apply the term to
        // a determinant with exactly the involved orbitals occupied. A nonzero
        // sign proves the term is a pure product of number operators (n_i, n_i
        // n_j, ...) with an occupancy-independent constant sign; anything that
        // needs an empty orbital (e.g. c_i c^d_i = 1 - n_i) fails the probe and
        // is left to the general diagonal path. So correctness never depends on
        // this firing.
        ...
        ManyBodyState::key_type
            mask; // occupancy mask over the involved orbitals
        std::complex<double> signed_coeff{0.0, 0.0};
        if (is_density) {
          const size_t num_bits =
              8 * sizeof(ManyBodyState::key_type::value_type);
          for (int64_t o : creators) {
            const size_t chunk = static_cast<size_t>(o) / num_bits;
            const size_t bit =
                num_bits - 1 - (static_cast<size_t>(o) % num_bits);
            if (chunk >= mask.size()) {
          ...
          for (int64_t idx : indices) {
            const int s =
                idx >= 0 ? create(probe, static_cast<size_t>(idx))
        ...
        // Off-diagonal one-body hop c^d_i c_j (i != j): exactly one creator and
        // one annihilator on different orbitals (Phase 2c). Precompute the
        // between-mask so the fermion sign is one popcount at apply time.
        ...
        if (is_onebody) {
          ...
          for (size_t o = lo + 1; o < hi; o++) {
            ...
            between[chunk] |=
                static_cast<ManyBodyState::key_type::value_type>(1) << bit;
          }
        }
