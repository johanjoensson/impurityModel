    ...
    def __init__(
        ...
        if blocked:
            self.blocks = get_block_structure(
                ...
                np.conj(corr_to_cf.T) @ h_dft @ corr_to_cf,
            )
...
def run_impmod_ed(
    ...
    u4 = np.einsum("ij,kl, jlmo, mn, op", np.conj(corr_to_cf.T),
                   np.conj(corr_to_cf.T), u4, corr_to_cf, corr_to_cf)
