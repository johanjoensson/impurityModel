...
Layer 0: average, utils, config, polarization, product_state_representation, op_parser,
         mpi_comm, solver_trace, ManyBodyUtils (Cython)
...
Layer 5: gf_primitives, gf_convergence, gf_shift_recycling, gf_units, gf_solvers,
         ...
         gs_statistics, dc_reference, dc_static, dc_search, dc_criteria, dc_frozen,
         double_counting, sigma, model
...
- **`atomic_physics.py`** — single-shell atomic physics: Slater–Condon Coulomb integrals (`getU*`), spin-orbit coupling (`getSOCop`), Zeeman field (`gethHfieldop`), spherical↔cubic transforms, the MLFT double-counting correction (`dc_MLFT`), and the average Coulomb repulsion/exchange extractor (`uj_from_u4`) the `double_counting` FLL/AMF schemes derive `U`/`J` from.
...
- **`dc_reference.py`** — the DFT reference filling every double-counting scheme measures against: the Fermi occupation (`_noninteracting_impurity_occupation`) and density matrix (`_noninteracting_impurity_rho`) of the **raw** `h0`. `model.h0` is the KS/DFT Hamiltonian of the `h0 - dc + U` contract, so its raw filling *is* the DFT occupation; filling `h0 - dc` instead sinks the impurity levels below E_F and saturates the reference at the full shell (NiO: `n0` pinned at 10 instead of ~8.6 for a ~4 Ry dc). Because the reference is a property of the *discretized* bath it saturates whenever the fit places no impurity weight across E_F, which for FLL is an O(1)-electron, several-eV error made silently -- so the saturation check lives here, with the reference, and every scheme that defaults to it routes through `_warn_if_reference_saturated`. Deterministic NumPy on the replicated `h0`: no MPI collective, identical on every rank.
- **`dc_static.py`** — the three closed-form schemes, each evaluated at `dc_reference`'s filling unless the caller supplies one: `fll_dc` (Fully Localized Limit), `amf_dc` (Around Mean Field) and `sigma_inf_dc` (K. Held's `Sigma(infinity)`, the full static Hartree-Fock self-energy matrix at the actual density matrix, `sigma.get_Sigma_static`). No ED solve and no MPI collective. `atomic_physics.uj_from_u4` derives the average Coulomb repulsion/exchange FLL and AMF need from `model.u4`.
- **`dc_search.py`** — `_solve_dc_shift`, the root find both ED criteria share, over the uniform shift `dc(mu) = dc_guess + mu * I`: a bidirectional geometric scan with safeguarded secant/bisection refinement (`_refine_bracket`). It knows nothing about what the observable is, so it carries no ED machinery and imports no solver. Two properties are load-bearing: **no monotonicity is assumed** (the residual can be a difference of two independently-monotone quantities, and the criteria re-select both the CIPSI space and the charge sector at every `mu`), and the residual -- the value of a *collective* observable, replicated only to roundoff -- is **broadcast once, in one place**, because every branch it gates decides whether the next collective call happens. Also the search's cost accounting (`_report_dc_trace`, the `DC_DIAGNOSTICS` knob), which reports the per-`mu` build/expand/eigensolve split and `chi = d(observable)/dmu`, the slope that converts a residual into an error on the answer.
- **`dc_criteria.py`** — the two ED-based criteria built on that search: `fixed_peak_dc` pins a spectral peak (the sector-energy difference `E[N+1] - E[N]`, or `E[N] - E[N-1]` for a removal peak), `fixed_occupation_dc` pins the impurity occupation, and with no explicit target pins it to `dc_reference`'s DFT occupation -- the criterion for CSC DFT+DMFT of wide-window p-d models, where only the d electrons enter the ED solve but the p electrons sit inside the DFT window. At every trial shift both determine the ground-state sector the *identical* way `groundstate.calc_gs` does (`find_ground_state_basis`'s HF-seed-then-walk search, not a search pinned at the input occupation): a dc measured on a different sector than the one `calc_selfenergy` later finds locks the downstream calculation onto the wrong charge state, which is worse than not fixing anything. Memory-limited like `calc_selfenergy`/`find_ground_state_basis` -- a `None` `truncation_threshold` derives the determinant cap from available per-rank memory (`memory_estimate.suggest_truncation_threshold`, halved for `fixed_peak_dc`'s co-resident upper/lower sector bases) -- and both honor `BasisOptions.excitation_budget`/`chain_restrict` via `basis_restrictions.build_weighted_restrictions`, so the double counting is found on the same variational space the production solve uses.
- **`dc_frozen.py`** — `FrozenSpaceSweep`: `energy(mu)` / `occupation(mu)` / `chi(mu)` over a *fixed* determinant space. Because the double counting is a uniform impurity shift, `H(mu) = H(0) - mu*N_imp` with `N_imp` **diagonal**, so the whole family is one matrix build plus a diagonal shift — the CIPSI expansion, 96.5% of a real DC search, is skipped entirely. Measured on NiO-20 at cap 400: a true re-expanding evaluation 3.41 s, one frozen solve **0.003 s** (0.08%, ~1240x). That is what makes `chi = dn/dmu` affordable, and `chi` is what turns an occupation residual into the error on the answer (`delta_mu = delta_n / chi`; on a plateau `chi -> 0` and `dc` is unbounded). On a frozen space the theory is exact — `E_0(mu)` is a min of affine functions, hence concave, with `n(mu)` non-decreasing, and on a *pure* sector the shift is affine to machine precision — none of which holds for the re-expanding observable, which is why the production search assumes no monotonicity. The frozen space is a cheap *model* of the answer, not the answer: it bounds the reachable occupation by its own charge window (`occupation_is_interior` reports that), so a root located here must be confirmed against the true observable. Solves route through `CIPSISolver.get_eigenvectors(h_matrix=...)` rather than around it, so the warm-start cold-retry guard still applies.
- **`double_counting.py`** — re-export shim over the four `dc_*` modules, so existing callers and `selfenergy` (which re-exports again) are unchanged. Note that **a re-exported name is a new binding**: patching `double_counting.X` does not affect the call site inside `dc_criteria`, which reads its own. Tests stubbing a dependency must patch the module that looks the name up at call time, and prove the stub ran -- a dead patch leaves an assertion like `assert not calls` passing vacuously. Split out of `selfenergy.py`, then out of itself.
