...
def test_getNIXSOperator(mock_jn, mock_simpson):
    ...
    nBaths = OrderedDict([(2, 10)])
    op = spectra.getNIXSOperator(nBaths, [1,1,1], 2, 2, np.array([1.]), np.array([1.]), np.array([1.]), kmin=1)
    assert isinstance(op, dict)
...
def test_get_noninteracting_hamiltonian_operator(mock_read_h0):
    ...
    h0 = get_spectra.get_noninteracting_hamiltonian_operator({2: 10, 1: 6}, {2: 10, 1: 6}, (0.1, 0.1), (0., 0., 0.), 'dummy.pickle', 0, False)
    assert isinstance(h0, dict)
