...
void pack_psis(const std::vector<const ManyBodyState *> &psis, int comm_size,
               ...
               std::vector<uint64_t> &state_buf, std::vector<double> &amp_buf,
...
// [state: chunks_per_state x uint64][amp: 2 x double][psi_idx: int32] so the
// whole redistribute moves in ONE Neighbor_alltoallv(MPI_BYTE) instead of
// three. At 100s-1000s of ranks the dense personalized exchange is
// latency-bound, so collapsing 3 message rounds into 1 cuts the dominant cost
// ~3x. send_buf is rank-ordered; bytes_per_entry = chunks_per_state*8 + 20.
