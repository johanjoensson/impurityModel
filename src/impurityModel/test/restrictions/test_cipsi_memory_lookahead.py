...
def test_bound_scales_the_measured_transient_by_basis_and_width_growth():
    ...
    assert affordable(100 * MiB, 400 * MiB) == (2000, "transient")


def test_bound_returns_zero_when_even_the_same_size_does_not_fit():
    affordable = _memory_growth_bound(1000 * MiB, 1000, 10, 10)
    # 400 + 700 > 1000: headroom is positive, but this round's own cost overruns it -- a
    # *measured* zero, which `expand` acts on at the first strike.
    assert affordable(700 * MiB, 400 * MiB) == (0, "transient")
    # No headroom at all: the verdict is the budget's, not this round's, and `expand` must wait
    # out the two-round streak before acting on it.
    assert affordable(100 * MiB, 1000 * MiB) == (0, "budget")


def test_bound_has_no_opinion_without_a_measurement():
    affordable = _memory_growth_bound(1000 * MiB, 1000, 10, 10)
    assert affordable(-1, 400 * MiB) == (None, "unmeasured")
    assert affordable(0, 400 * MiB) == (None, "unmeasured")
...
def test_a_budget_side_zero_is_reported_as_the_budgets_verdict_not_the_rounds(monkeypatch):
    """When the resident set alone is over budget the round's own cost never enters into it.

    This is the shape that pinned every SrMnO3 gap-DC sector after the first at its seed basis
    (`doc/plans/dc_smo_memory.md`, round 9) -- there the budget was the *free*-memory fraction,
    so a 4-68 KiB round read as unaffordable. With an absolute budget the same code path now
    only triggers when the process really does hold its whole share, and it is labelled as such
    so a log can tell the two apart.

    It still caps on the first strike, and must: `rss_base >= budget` implies
    `peak_rss >= budget` (VmHWM was reset to the round's starting RSS), so the after-the-fact
    trip-wire -- the OOM backstop -- has already fired by the time this is visible. A separate
    streak for this branch would be unreachable code.
    """
    H = _hamiltonian()
    solver = _make_solver(None)
    # Resident set already above the budget, and a transient of 1 byte per determinant: the round
    # costs nothing, and the bound never gets as far as asking what it cost.
    _FakeMemory(monkeypatch, solver, bytes_per_det=1, floor=500 * MiB)
    solver.expand(H, de2_min=GS_DE2_MIN, solver="trlm", memory_budget_bytes=100 * MiB)

    assert solver.last_selection["memory_cap_reason"] == "budget"
    assert solver.last_selection["memory_admit_cap"] == 0
    assert np.isfinite(solver.basis.truncation_threshold)
    assert solver.truncation_report is not None and solver.truncation_report["memory_bound"]


def test_a_measured_zero_still_caps_immediately(monkeypatch):
    """The counterpart: when the round's own measured cost is what overruns the headroom, there
    is no second round to wait for -- the expansion admits nothing, the loop terminates, and the
    cap must be adopted on the first strike."""
    H = _hamiltonian()
    solver = _make_solver(None)
    # 400 MiB/determinant above a 100 MiB floor against a 1 GiB budget: headroom is positive, so
    # the bound consults the transient, and the transient overruns it.
    _FakeMemory(monkeypatch, solver, bytes_per_det=400 * MiB)
    solver.expand(H, de2_min=GS_DE2_MIN, solver="trlm", memory_budget_bytes=1024 * MiB)

    assert solver.last_selection["memory_cap_reason"] == "transient"
    assert np.isfinite(solver.basis.truncation_threshold)
    assert solver.truncation_report is not None and solver.truncation_report["memory_bound"]
