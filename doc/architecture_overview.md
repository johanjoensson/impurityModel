...
- **`double_counting.py`** — double-counting determination for the self-energy workflow; `model.dc` is a first-class part of `ImpurityModel`, always subtracted (`h0 - dc + U(u4)`, never added). Two ED-based searches over a uniform shift `dc(mu) = dc_guess + mu * I`: `fixed_peak_dc` (pin a spectral peak) and `fixed_occupation_dc` (pin the impurity occupation, or with no explicit target pin it to the DFT reference occupation `N0` -- the Fermi filling of the raw `h0`, which is the KS Hamiltonian of the `h0 - dc + U` contract; never of `h0 - dc`, which saturates the reference at the full shell), both driven by one non-monotonicity-safe bidirectional bracketing search (`_solve_dc_shift`) rebuilding the variational ground state and its thermal density matrix at each trial shift. Memory-limited like `calc_selfenergy`/`find_ground_state_basis`: a `None` `truncation_threshold` derives the determinant cap from available per-rank memory (`memory_estimate.suggest_truncation_threshold`, halved for `fixed_peak_dc`'s co-resident upper/lower sector bases), and both honor `BasisOptions.excitation_budget`/`chain_restrict` via `basis_restrictions.build_weighted_restrictions` so the double counting is found on the same variational space the production solve uses. Three static schemes need no ED solve and no MPI collective (deterministic one-body NumPy, identical on every rank): `fll_dc` (Fully Localized Limit), `amf_dc` (Around Mean Field) and `sigma_inf_dc` (K. Held's `Sigma(infinity)`, the full static Hartree-Fock self-energy at the actual density matrix, `sigma.get_Sigma_static`); each evaluates its formula at the DFT occupation/density matrix (the raw-`h0` Fermi filling, the same reference `fixed_occupation_dc` targets), unless the caller supplies it explicitly. `atomic_physics.uj_from_u4` derives the average Coulomb repulsion/exchange FLL/AMF need from `model.u4`. Split out of `selfenergy.py`.
