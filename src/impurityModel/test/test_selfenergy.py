...
def test_get_hcorr_v_hbath_reversed():
    ...
    assert v_dagger[1, 0] == -0.3 # n_corr=2, so j=2 corresponds to col 0 of v_dagger. wait, h0Matrix[1, 2] is hcorr vs v_dagger?
...
def test_calc_selfenergy(mock_get_gf, mock_calc_gs):
    ...
    mock_get_gf.return_value = (
        [np.array([[[-1j]]]), None], # gs_matsubara
        [np.array([[[-1j]]]), None]  # gs_realaxis
    )
...
def test_fixed_peak_dc_negative(mock_getUop, mock_Basis, mock_CIPSISolver, mock_thermal_avg):
    ...
    dc = selfenergy.fixed_peak_dc(
        h0_op={}, N0={0: 1}, mixed_valence=False, impurity_orbitals={0: [[0]]},
        bath_states=({0: []}, {0: []}), u4=np.zeros((1, 1, 1, 1)),
        peak_position=-0.1, dc_guess=np.array([[1.0]]), spin_flip_dj=False,
        tau=-1.0, rank=0, verbose=True, dense_cutoff=100, slaterWeightMin=1e-12, truncation_threshold=100
    )
    assert dc is not None
...
def test_calc_selfenergy_no_matsubara(mock_get_gf, mock_calc_gs):
    mock_calc_gs.return_value = (
        [np.array([1.0])], [0.0], MagicMock(restrictions=None, impurity_orbitals={0: [[0]]}), np.array([[1.0]]), {"rhos": [np.array([[1.0]])]}
    )
    mock_get_gf.return_value = (
        None, # gs_matsubara
        [np.array([[[-1j]]]), None]  # gs_realaxis
    ...
    res = selfenergy.calc_selfenergy(
        h0={((0, "c"), (0, "a")): 1.0}, u4=np.zeros((1, 1, 1, 1)), iw=np.array([1j]), w=np.array([0.0]),
        delta=0.1, nominal_occ={0: 1}, mixed_valence=False, impurity_orbitals={0: [[0]]},
        bath_states=({0: []}, {0: []}), tau=0.1, verbosity=2,
        block_structure=BlockStructure([[0]], [[0]], [[]], [[]], [[]], [0]), rot_to_spherical=np.eye(1),
        cluster_label="test", reort=None, dense_cutoff=100, spin_flip_dj=False, comm=None,
        chain_restrict=False, occ_cutoff=1e-12, truncation_threshold=100, slaterWeightMin=1e-12, dN=None, sparse_green=False
    )
...
def test_calc_selfenergy_exceptions(mock_get_gf, mock_calc_gs):
    ...
    mock_calc_gs.return_value = (
        [np.array([1.0])], [0.0], MagicMock(restrictions={((0,),): (0, 1)}, impurity_orbitals={0: [[0]]}), np.array([[1.0]]), {"rhos": [np.array([[1.0]])]}
    )
...
def test_get_selfenergy_exception(mock_getUop, mock_get_h0, mock_calc):
    ...
    mock_getUop.return_value = {(((99, 99, 99), "c"), ((99, 99, 99), "a"), ((99, 99, 99), "c"), ((99, 99, 99), "a")): 1.0}
